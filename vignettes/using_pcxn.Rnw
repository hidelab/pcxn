...
# the 10 most correlated neighbors and specific cut-offs for minimum absolute 
...
Draw a heatmap of a pcxn object produced by pcxn_explore() or pcxn.analyse(). A
...
Create a network visualisation of a pcxn object produced by pcxn_explore()
or pcxn_analyze().
