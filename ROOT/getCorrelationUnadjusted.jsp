...
<%
	...
	if (geneSetCollectionValue.equals("pathprint")) {
		tblName = "pathprint_correlation_tbl_unadjusted";
	} else if (geneSetCollectionValue.equals("MSigDBC2CP")) {
		tblName = "msigdb_c2_cp_correlation_tbl_unadjusted";
	} else if (geneSetCollectionValue.equals("MSigDBC5")) {
		tblName = "msigdb_c5_go_correlation_tbl_unadjusted";
	} else if (geneSetCollectionValue.equals("MSigDBH")) {
		tblName = "msigdb_h_hallmark_correlation_tbl_unadjusted";
	} else {
		tblName = null;
	}
	
	String JDBC_DRIVER = "com.mysql.jdbc.Driver";  
	String DB_URL = "jdbc:mysql://localhost/pcxn_database_yered?autoReconnect=true&useSSL=false"; //changed
	...
	if (topNValue > 0){
		...
		String sql1 = "select gene_set_B as gene_set_C, max(abs(correlation)) as abs_correlation from " + tblName + " where gene_set_A in ( " + genesetsText + ") AND gene_set_B not in (" + genesetsText + ") AND p_value < " + pCutOffValue + " AND abs(correlation) > " + corrCutOffValue + " group by gene_set_C union select gene_set_A as gene_set_C, max(abs(correlation)) as abs_correlation from " + tblName + " where gene_set_A not in ( " + genesetsText + ") AND gene_set_B in (" + genesetsText + ") AND p_value < " + pCutOffValue + " AND abs(correlation) > " + corrCutOffValue + " group by gene_set_C order by abs_correlation desc";
	...
	String sql2 = "select gene_set_A, gene_set_B, correlation, p_value, overlap_coefficient from " + tblName + " where gene_set_A in ( " + genesetsText + ") AND gene_set_B in (" + genesetsText +")";
